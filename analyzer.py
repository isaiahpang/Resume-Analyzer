...
def extract_resume_profile(resume_text: str) -> dict:
    ...
    user = f"RÉSUMÉ TEXT:\\n\\n{resume_text}"
    return ask_json(RESUME_PROFILE_PROMPT, user, temperature=0.0, max_tokens=2000)
...
def extract_jd_profile(jd_text: str) -> dict:
    ...
    return ask_json(JD_PROFILE_PROMPT, user, temperature=0.0, max_tokens=1500)
...
def analyse_bullets(resume_profile: dict) -> dict:
    ...
    user = f"RÉSUMÉ PROFILE:\\n{json.dumps(resume_profile, indent=2)}"
    return ask_json(BULLET_QUALITY_PROMPT, user, temperature=0.2, max_tokens=3000)
...
def analyse_jargon(
    ...
    return ask_json(JARGON_AUDIT_PROMPT, user, temperature=0.2, max_tokens=1500)
...
def analyse_degree_alignment(jd_profile: dict, degree_program: str) -> dict:
    ...
    user = f"DEGREE PROGRAM: {degree_program}\n\nJD PROFILE:\n{json.dumps(jd_profile, indent=2)}"
    return ask_json(DEGREE_ALIGNMENT_PROMPT, user, temperature=0.2, max_tokens=600)
...
def summarise_overall(report: dict) -> str:
    ...
    return ask_text(OVERALL_SUMMARY_PROMPT, user, temperature=0.2,max_tokens=400)
